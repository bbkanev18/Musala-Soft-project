...
void BoxPrintTeam(TEAM* team, size_t indent, bool inlineStudents)
{
	...
	PrintStudentVector(team->students, indent + 2, inlineStudents, inlineStudents);
...
void InlinePrintTeam(TEAM* team, size_t indent, int id)
{
	std::vector<std::wstring> content;
	int n = 0;
...
TEAM* CreateSampleTeam(std::vector<std::wstring>& teamNames, std::vector<std::wstring>& names, std::vector<std::wstring>& surnames, bool empty)
{
	...
	CreateSampleStudentVector(names, surnames, team->students, 4 * !empty + 0);
...
void RemovePointerFromVector(std::vector<TEAM*>& vec, size_t posStart, size_t posEnd)
{
	if (posEnd != 0)
		vec.erase(vec.begin() + posStart, vec.begin() + posEnd);
	vec.erase(vec.begin() + posStart);
...
void PrintTeamVector(std::vector<TEAM*>& vec, size_t indent, bool inlineTeams, bool inlineStudents, bool index)
{
	NewLine(inlineTeams);
...
void AddPointerToVector(std::vector<TEAM*>& vec, TEAM* obj)
{
	vec.push_back(obj);
}

void ClearPointerVector(std::vector<TEAM*>& vec)
{
	for (size_t i = 0; i < vec.size(); i++)
		delete vec[i];
	vec.clear();
...
std::vector<TEAM*> CombineTeamVectors(std::vector<TEAM*>& tchTemVec, std::vector<TEAM*>& tchlessTeams)
{
...
void BoxPrintTeacher(TEACHER* tch, size_t indent)
{
	//Symbols used for box
	//─┤├──┠┨┎┒┃┖┚┄
...
void InlinePrintTeacher(TEACHER* tch, size_t indent, int id)
{
	std::vector<std::wstring> content;
...
TEACHER* EnterTeacher(std::vector<TEAM*>& tchlessTeams, std::vector<TEAM*>& tchTemVec)
{
	...
	do
	{
		...
		std::wcin.clear();
		//BREAKS THE PROGRAM WHEN THE USER INPUTS -1 AND THE TCH.TEAM*S IS EMPTY
...
TEACHER* CreateSampleTeacher(std::vector<std::wstring>& names, std::vector<std::wstring>& surnames, std::vector<TEAM*>& tchlessTeams, std::vector<TEAM*>& tchTemVec)
{
	TEACHER* tch = new TEACHER();
	std::wstring email = L"";
...
void AddPointerToVector(std::vector<TEACHER*>& vec, TEACHER* obj)
{
	vec.push_back(obj);
}

void RemovePointerFromVector(std::vector<TEACHER*>& vec, size_t posStart, size_t posEnd)
{
	if (posEnd != 0)
		vec.erase(vec.begin() + posStart, vec.begin() + posEnd);
	vec.erase(vec.begin() + posStart);
...
void PrintTeacherVector(std::vector<TEACHER*>& vec, size_t indent, bool inlineTeacher, bool index)
{
...
SCHOOL* EnterSchool(std::vector<std::wstring>& teamNames, std::vector<std::wstring>& names, std::vector<std::wstring>& surnames)
{
	...
	for (size_t i = 0; i < sch->teams.size(); i++)
		for (size_t j = 0; j < sch->teams[i]->students.size(); j++)
			AddPointerToVector(sch->students, sch->teams[i]->students[j]);
